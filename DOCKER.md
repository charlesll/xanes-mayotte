...
#### build_image()
```bash
docker build -t mayotte-xas:1.0.5 .
```
...
docker run --rm \
    ...
    -v "$(pwd)/figures:/home/xasuser/mayotte/figures" \
    -v "$(pwd)/results:/home/xasuser/mayotte/results" \
    mayotte-xas:1.0.5 python modelling.py
```
...
7. **Image Creation**: Final image tagged as `mayotte-xas:1.0.5`
...
[+] Building 187.3s (15/15) FINISHED
 ...
 => exporting to image
 => => naming to docker.io/library/mayotte-xas:1.0.5
```
...
docker build --progress=plain -t mayotte-xas:1.0.5 .
...
docker run --rm mayotte-xas:1.0.5 ls -lh /home/xasuser/mayotte/src/ctsfg6
...
docker run --rm \
    ...
    -v $(pwd)/figures:/home/xasuser/mayotte/figures \
    -v $(pwd)/results:/home/xasuser/mayotte/results \
    mayotte-xas:1.0.5 python -u analysis_publication.py
```
...
docker run --rm \
    ...
    -v $(pwd)/figures:/home/xasuser/mayotte/figures \
    -v $(pwd)/results:/home/xasuser/mayotte/results \
    mayotte-xas:1.0.5 python analysis_publication.py
```
...
docker build --no-cache-filter pip -t mayotte-xas:1.0.5 .
```
...
# Full rebuild
docker build --no-cache -t mayotte-xas:1.0.5 .
```
...
# Keep old version
docker tag mayotte-xas:1.0.5 mayotte-xas:1.0.5-backup
...
```bash
# Save image to file
docker save mayotte-xas:1.0.5 | gzip > mayotte-xas-1.0.5.tar.gz

# Transfer file to another machine, then:
docker load < mayotte-xas-1.0.5.tar.gz
```
